...
`org.toml` の `role` は「誰が意思決定し、誰が実装し、誰が承認するか」を表します。
（運用ポリシーの強制は段階的に実装中）

- `boss`（社長）
  - 意思決定者。**作業しない**。
  - main へのマージなど重大判断を行う想定。
- `ghost_boss`（取締役会）
  - 社長の意思決定を補助する人格群（多数決/壁打ち用）。
- `manager`（部長）
  - 複数課へのディスパッチャー。
  - 課ブランチ（team branch）を main にマージして良いか判断する想定。
- `lead`（課長）
  - 課のレビュー責任者。
  - ワーカー成果を承認し、課ブランチへ反映して良いか判断する想定。
- `worker`（ワーカー）
  - 作業者。Docker sandbox + worktree で実装する想定。
- `reviewer`（互換）
  - 旧ロール互換用（過去のテスト/サンプル向け）。将来的には `lead` に寄せる想定。

## ブランチ命名（方針）

課（`lead`）単位でブランチを作ります（フラット命名）。

- 課ブランチ: `team-<lead_id>`
  - 例: `team-dev_lead`

ワーカーの作業ブランチは別名で切り、承認後に課ブランチへ反映する想定です。
